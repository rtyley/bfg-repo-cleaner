...
//
//
//    contain(name) compose { (treeish: ObjectId) =>
//    treeOrBlobPointedToBy(treeish.asRevObject) match {
...
//    // repo.git.gc.call() ??
//    repo should (dirtMatchers.reduce(_ and _))
//    block
//    // repo.git.gc.call() ??
//    repo should dirtMatchers.map(not(_)).reduce(_ and _)
//  }

  class Boom(commits: => Seq[RevCommit]) extends Inspectors {
...
//  def ensureRemovalOf[T](dirtMatchers: Matcher[Repository]*)(block: => T) = {
//    // repo.git.gc.call() ??
//    withClue("this is a before clue") {
//      repo should (dirtMatchers.reduce(_ and _))
//    }
//    block
//    // repo.git.gc.call() ??
//    withClue("this is a after clue") {
//      repo should dirtMatchers.map(not(_)).reduce(_ and _)
//    }
//  }
  ...
  def ensureInvarint[T](repoMatcher: Matcher[Repository])(block: => T) = {
    repo should repoMatcher
    block
    repo should repoMatcher
  }
}
